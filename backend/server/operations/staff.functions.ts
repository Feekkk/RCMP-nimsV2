...
import type { CreateStaffInput, UpdateStaffInput } from '@shared/lib/staff-schema';
...
export const createStaffFn = createServerFn({ method: 'POST' })
  ...
  .handler(async ({ data }) => {
    const { createStaff } = await import('@backend/server/operations/staff-repo.server');
...
export const updateStaffFn = createServerFn({ method: 'POST' })
  ...
  .handler(async ({ data }) => {
    const { updateStaff } = await import('@backend/server/operations/staff-repo.server');
...
export const listStaffHandoverAssetsFn = createServerFn({ method: 'GET' })
  ...
  .handler(async ({ data: employeeNo }) => {
    const { listStaffHandoverAssets } = await import('@backend/server/operations/staff-repo.server');
