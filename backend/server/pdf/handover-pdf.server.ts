...
import { loadLogoBase64 } from '@backend/server/pdf/pdf-form-common.server';
...
export async function generateHandoverPdfBuffer(handoverId: number): Promise<Uint8Array> {
  const { getHandoverNotificationData } = await import('@backend/server/pdf/handover-pdf-repo.server');
