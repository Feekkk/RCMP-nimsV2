...
import {
  ...
  requireUser,
  revokeRefreshToken,
} from '@backend/server/auth/api-auth.server';
import { apiError, apiOk, handleApiError, readJsonBody } from '@backend/server/core/api-response.server';
...
export async function handleRefresh(request: Request): Promise<Response> {
  try {
    ...
    const { getAuthUserByStaffId } = await import('@backend/server/auth/auth-repo.server');
    const { verifyRefreshTokenSubject } = await import('@backend/server/auth/api-auth.server');
...
export async function handleDevLogin(request: Request): Promise<Response> {
  ...
  try {
    ...
    const { devLoginAsTechnician, devLoginAsAdmin, devLoginAsUser, devLoginAsDisposalUnit } = await import(
      '@backend/server/auth/auth-repo.server'
    );
    let user;
...
export async function handleGetProfile(request: Request): Promise<Response> {
  try {
    const auth = await requireAuth(request);
    if (auth instanceof Response) return auth;
    const { getStaffProfile, getUserProfile, getAuthUserByStaffId } = await import('@backend/server/auth/auth-repo.server');
...
export async function handlePatchProfile(request: Request): Promise<Response> {
  try {
    ...
    if (body instanceof Response) return body;
    const { updateStaffProfile, updateUserProfile, getAuthUserByStaffId } = await import('@backend/server/auth/auth-repo.server');
...
export async function handleListStaff(request: Request): Promise<Response> {
  try {
    const auth = await requireStaff(request);
    if (auth instanceof Response) return auth;
    const { listStaffDirectory } = await import('@backend/server/operations/staff-repo.server');
...
export async function handleRequestPool(request: Request): Promise<Response> {
  try {
    ...
    const { listRequestPoolAssets, listAvailablePoolAssets, listAssignedRequestPoolAssets } =
      await import('@backend/server/requests/request-repo.server');
