...
import type { SendNotificationEmailInput } from '@shared/lib/email-notification';
...
export const verifyEmailConfigFn = createServerFn({ method: 'GET' })
  .middleware([staffMiddleware])
  .handler(async () => {
    const { isEmailConfigured } = await import('@backend/lib/microsoft-email-config');
    ...
    try {
      const { verifyEmailTransport } = await import('@backend/server/email/email.server');
