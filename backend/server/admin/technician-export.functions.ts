...
import type { TechnicianAssetExportKind, TechnicianReportPdfFilters } from '@shared/lib/technician-export-schema';
...
export const exportTechnicianAssetCsvFn = createServerFn({ method: 'POST' })
  ...
  .handler(async ({ data }) => {
    ...
    const { exportTechnicianAssetCsv } = await import('@backend/server/admin/technician-export.server');
...
export const generateAssetReportPdfFn = createServerFn({ method: 'POST' })
  ...
  .handler(async ({ data }) => {
    ...
    const { generateAssetReportPdfBase64 } = await import('@backend/server/pdf/asset-report-pdf.server');
