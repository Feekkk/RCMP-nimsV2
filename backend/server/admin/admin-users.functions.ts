...
import type { CreateAdminUserInput, UpdateAdminUserInput } from '@shared/lib/admin-users-schema';
...
export const listAdminUsersFn = createServerFn({ method: 'POST' })
  ...
  .handler(async () => {
    const { listAdminUsers } = await import('@backend/server/admin/admin-users-repo.server');
...
export const createAdminUserFn = createServerFn({ method: 'POST' })
  ...
  .handler(async ({ data }) => {
    const { createAdminUser } = await import('@backend/server/admin/admin-users-repo.server');
...
export const updateAdminUserFn = createServerFn({ method: 'POST' })
  ...
  .handler(async ({ data }) => {
    const { updateAdminUser } = await import('@backend/server/admin/admin-users-repo.server');
