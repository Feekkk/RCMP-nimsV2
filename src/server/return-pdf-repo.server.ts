...
import type { ReturnNotificationData } from '@/lib/return-pdf-types';
...
/**
 * Single query (+ Azure lookup only when the technician name wasn't captured at return time) shared
 * by both the return PDF and the return notification email — avoids duplicate DB/Graph round-trips.
 */
export async function getReturnNotificationData(
  returnId: number,
): Promise<ReturnNotificationData | null> {
  ...
  const [rows] = await pool.query<ReturnRow[]>(
    `SELECT hr.return_id, h.asset_id, hr.return_date, hr.\`condition\`, hr.return_remarks,
            ...
            hr.returned_by_name, u.oid AS returned_by_oid, r.name AS returned_by_role
  ...
  const returnedByName =
    row.returned_by_name?.trim() || (await getDisplayNameByOid(row.returned_by_oid));
